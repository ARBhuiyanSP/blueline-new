...
//        }
    ...
    if($error_status){
        ...
        $emailsql    = "SELECT * FROM users where username='$email'";
        $result = $conn->query($emailsql);
        if ($result->num_rows > 0) {
            $passsql    = "SELECT * FROM users where username='$email' AND password='$password'";
            $presult = $conn->query($passsql);
            if ($presult->num_rows > 0) {
                $row        	=   $presult->fetch_object();
                ...
                unset($_SESSION['error']);
                $_SESSION['success']                =   $name." have successfully loggedin!";
                $_SESSION['logged']['user_id']=   (isset($row->id) && !empty($row->id) ? $row->id : "");
                $_SESSION['logged']['username']    =   (isset($row->username) && !empty($row->username) ? $row->username : "");
